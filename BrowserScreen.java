...
public class BrowserScreen extends Canvas implements CommandListener {
    ...
    public static final String HOMEPAGE_URL = "http://www.rediffmail.com";
    PicoBrowser app;
    String httpMimeType;
    ...
    // command
    static final Command pgUp = new Command("PgUp", Command.BACK, 1);
    static final Command pgDn = new Command("PgDn", Command.SCREEN, 1);
    ...
    BrowserScreen(PicoBrowser app,String pt) {
  this.app = app;
	...
	this.addCommand(pgUp);
	this.addCommand(pgDn);
	...
	*/
	...
	for (int i = 0; i < items.size(); i++) {
		...
		if (item.line.ypos > (nPosY + nScreenHeight)) {
		    scrolldown = true;
		}
		break;
	    }
	...
	if (autoScroll && scrolldown) {
		...
		if (item.linkId != -1) {
		    // if it's not on the screen scroll one page forward
		    if ((nPosY <= item.line.ypos) && (item.line.ypos <= (nPosY + nScreenHeight))) {
	...
	// If no link found on this age, just scroll back one page.
	if ( !foundPrev) {
	    nPosY -= nScreenHeight;
	    if (nPosY < 0) {
		nPosY = 0;
    ...
    */
	...
	textInput.setWidget(w, maxSize, constraints);
